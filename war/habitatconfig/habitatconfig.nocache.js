...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "habitatconfig" because\n';
  ...
  $wnd.__gwt_activeModules['habitatconfig'] = {
    'moduleName' : 'habitatconfig',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/habitatconfig.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['habitatconfig'].superdevmode = true;

  var expectedSuffix = '/habitatconfig.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + habitatconfig + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:habitatconfig';
  var devModeSessionKey = '__gwtDevModeSession:habitatconfig';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/habitatconfig/habitatconfig.nocache.js';
  ...
  $wnd.__gwt_activeModules['habitatconfig'].canRedirect = true;
  $wnd.__gwt_activeModules['habitatconfig'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING habitatconfig';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/habitatconfig?user.agent=' + ua + '&_callback=' + callback;
